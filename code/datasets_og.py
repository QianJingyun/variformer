...
from torch.utils.data.distributed import DistributedSampler
        ...
        """ -----------------------------------------------prompt-----------------------------------------------
        I want to understand the code logic as a human in this github repository: https://github.com/shirondru/enformer_fine_tuning?tab=readme-ov-file. Let's do a Code Walkthrough. I aim to read three files:
        code/dataset.py (How it creates "Personal Genomes")
        code/train_gtex.py (The training loop)
        code/enformer.py or the usage of HeadAdapterWrapper (The actual neural net)

        You will also give me the big picture of how this function or class will be used. Use plain, easy language and terms from the paper (https://link.springer.com/article/10.1186/s13059-025-03926-7) for me to easy understand the big picture and relate back to the paper. Then, you are going to help me understand the code line-by-line. Be specific and explain the code, variable names, and why, for me to understand the code line by line. If you think this function is not important to understand for the big picture, notify me, and we can skip the second line-by-line explanation.
...
class GTExDataset(Dataset):
    def __init__(self, 
        ...
        """ -----------------------------------------------Batch VS Gradient Accumulation Batch-----------------------------------------------
        Batch size: It is the number of data samples your GPU can hold in its memory and process at one single moment.
        Example: processes 8 people's DNA at once.
        Gradient Accumulation Batch: It is a trick to simulate a larger batch size than your hardware allows. 
        Example: You might want the model to see 128 people's data before it updates its understanding (weights) to get a stable estimate of the error.
        -----------------------------------------------My comments end-----------------------------------------------"""
...
"""""-----------------------------------------------Shuffle logic-----------------------------------------------
        shuffle genomice_regions_df
        first for loop: sample the people
        randomly sample from individuals_in_split, the total qualified individuals, how many: per gene individuals * number of batches per gene.
        split into batches

        second for loop: map gene to people
        gene[i], individual[j]

        shuffle again: mix the genes. 
