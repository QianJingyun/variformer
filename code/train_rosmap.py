...
# from datasets import ROSMAPDataset
# torch.serialization.add_safe_globals([ROSMAPDataset])
# import torch
# from pandas import DataFrame
# torch.serialization.add_safe_globals([DataFrame])
...
# Maximize Tensor Core performance on A30
torch.set_float32_matmul_precision('high')

# Allowlist objects for PyTorch 2.6+ strict checkpoint loading
...
def load_rosmap_datasets(config,train_genes,valid_genes,test_genes):    # change 1 remove the GTEx logic and do train-test-val split on rosmap.
    rosmap_df_path = os.path.join(config.DATA_DIR,"rosmap_wgs","expression-rosmap.parquet") # change 2 the dirctory
    rosmap_gene_expression_df = pd.read_parquet(rosmap_df_path)
    ...
    train_ds = ROSMAPDataset(genes_to_train, config.seq_length, config.num_individuals_per_gene, config.train_donor_path,
                             rosmap_gene_expression_df, config.DATA_DIR)
    valid_ds = ROSMAPDataset(genes_to_validate, config.seq_length, -1, config.valid_donor_path, rosmap_gene_expression_df, config.DATA_DIR)
    test_ds = ROSMAPDataset(genes_to_test, config.seq_length, -1, config.test_donor_path, rosmap_gene_expression_df, config.DATA_DIR)
...
def train_rosmap(config: wandb.config,
    ...
    trainer.test(model, DataLoader(test_ds,batch_size = 1), ckpt_path = 'best', weights_only=False)
...
def main():
    ...
    for train_gene_filename in train_gene_filenames:
        ...
        #if not os.path.exists(save_dir): #run if this file doesn't already exist, to enable resuming if job runs out of time or crashes for some reason. # change 4, I teporarily commet this out and detent below for debug/rerun purposes.
        wandb.init(
